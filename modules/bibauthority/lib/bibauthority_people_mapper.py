...
class Mapper:
    ...
    def _create_datafield(
      self, parent, attr_code, attr_ind1=" ", attr_ind2=" ", repeatable=False):
    ...
    def _get_inspire_id(self, employee_id):
        """Get Inspire-ID by CERN employeeID.

        :param string employee_id: CERN-ID (employeeID)
        :return: Inspire-ID or None
        """
        return get_inspire_id_from_atlas_glance(employee_id)
    ...
    def map_ldap_record(self, record):
        ...
        for attr_key in self.mapper_dict.keys():
            ...
            if value:
                value = value[0].decode("utf-8", errors="replace")
                ...
                if attr_key is "employeeID":
                    value = "{0}{1}".format(CFG_BIBAUTHORITY_AUTHOR_CERN,
                                            value)
        ...
        if employee_id:
            ...
            if self.mapping_inspire_ids:
                inspire_id = get_inspire_id_from_atlas_glance_local(
                    employee_id, self.mapping_inspire_ids)
            else:
                inspire_id = get_inspire_id_from_atlas_glance(employee_id)

            if inspire_id:
                self._create_subfield(
                    self._create_datafield(elem_record, "035",
                                           repeatable=True),
                    "a",
                    "{0}{1}".format(CFG_BIBAUTHORITY_AUTHOR_INSPIRE,
                                    inspire_id))
    ...
    def _write_xml(self, tree, file):
        """Write tree to XML file.

        :param etree tree: XML tree to write
        :param filepath file: XML file to write to
        """
        try:
